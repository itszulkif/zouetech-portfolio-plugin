<?php
/**
 * Featured Portfolio Showcase — Style 1 template.
 *
...
if ( ! isset( $style ) ) {
	$style = Zouetech_Portfolio_Featured_Showcase_Styles::DEFAULT;
}
...
<div
	...
	tabindex="0"
>
	<div class="ztp-fs__featured">
		<?php if ( ! empty( $show['image'] ) ) : ?>
			<div class="ztp-fs__featured-media">
				<img
					class="ztp-fs__featured-img"
					src="<?php echo esc_url( $featured['image']['url'] ); ?>"
					alt="<?php echo esc_attr( $featured['title'] ); ?>"
					data-ztp-featured-img
					decoding="async"
				/>
			</div>
		<?php endif; ?>

		<div class="ztp-fs__featured-content">
			<div class="ztp-fs__copy" data-ztp-copy>
				...
				<?php if ( ! empty( $show['title'] ) ) : ?>
					<h2 class="ztp-fs__title" data-ztp-title aria-live="polite">
			...
			<?php if ( ! empty( $show['gallery'] ) ) : ?>
				...
				<div
					class="ztp-fs__gallery<?php echo $ztp_has_gallery ? '' : ' is-empty'; ?>"
					data-ztp-gallery
					<?php echo $ztp_has_gallery ? '' : 'hidden'; ?>
				>
					<button type="button" class="ztp-fs__gallery-arrow ztp-fs__gallery-arrow--prev" data-ztp-gallery-prev aria-label="<?php esc_attr_e( 'Scroll gallery left', 'zouetech-portfolio' ); ?>" hidden>
					...
					<div class="ztp-fs__gallery-viewport">
						<div class="ztp-fs__gallery-track" data-ztp-gallery-track>
							<?php foreach ( $featured['gallery'] as $g_i => $g_item ) : ?>
								<button
									type="button"
									class="ztp-fs__thumb<?php echo ! empty( $g_item['is_featured'] ) ? ' ztp-fs__thumb--featured' : ''; ?><?php echo 0 === (int) $g_i ? ' is-active' : ''; ?>"
									data-ztp-thumb
									data-index="<?php echo esc_attr( (string) $g_i ); ?>"
									data-url="<?php echo esc_url( $g_item['url'] ); ?>"
									aria-label="<?php echo esc_attr( sprintf( /* translators: %d image number */ __( 'Gallery image %d', 'zouetech-portfolio' ), $g_i + 1 ) ); ?>"
									<?php echo 0 === (int) $g_i ? 'aria-current="true"' : ''; ?>
								>
									<img
										src="<?php echo esc_url( $g_item['thumb'] ); ?>"
										alt=""
										loading="<?php echo ( $g_i < 2 ) ? 'eager' : 'lazy'; ?>"
										decoding="async"
									/>
								</button>
							<?php endforeach; ?>
					...
					<button type="button" class="ztp-fs__gallery-arrow ztp-fs__gallery-arrow--next" data-ztp-gallery-next aria-label="<?php esc_attr_e( 'Scroll gallery right', 'zouetech-portfolio' ); ?>" hidden>
						<span aria-hidden="true">›</span>
					</button>
				</div>
			<?php endif; ?>
			...
			<?php if ( ! empty( $show['nav'] ) ) : ?>
				<div class="ztp-fs__nav">
					<button type="button" class="ztp-fs__nav-btn ztp-fs__nav-btn--prev" data-ztp-prev aria-label="<?php echo esc_attr( $labels['back'] ); ?>">
						<?php echo esc_html( $labels['back'] ); ?>
					</button>
					<button type="button" class="ztp-fs__nav-btn ztp-fs__nav-btn--next" data-ztp-next aria-label="<?php echo esc_attr( $labels['next'] ); ?>">
	...
	<?php if ( ! empty( $show['cards'] ) && ! empty( $cards ) ) : ?>
		<div class="ztp-fs__cards" data-ztp-cards>
